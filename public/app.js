...
async function renderSessionsPage() {
  ...
  el.innerHTML = state.sessions.length ? `<div class="card"><table class="tbl"><tr><th>Session</th><th>Persona</th><th>Messages</th><th>Updated</th><th></th></tr>${state.sessions.map((s) => `<tr><td><b>${esc(s.name)}</b></td><td>${esc(personaName(s.persona))}</td><td>${s.message_count}</td><td>${fmtWhen(s.updated_at)}</td><td class="num"><button class="btn sm pri" data-open="${s.id}">Open</button> <a class="btn sm" href="/api/sessions/${s.id}/export?format=md" download>.md</a> <a class="btn sm" href="/api/sessions/${s.id}/export" download>.json</a> <button class="btn sm danger" data-del="${s.id}">Delete</button></td></tr>`).join("")}</table></div>` : `<div class="card"><div class="hint">No saved sessions yet. Save the current conversation from the Sessions ▾ menu in Chat.</div></div>`;
...
function browse() {
  return new Promise((resolve) => {
    ...
    async function go(p) {
      ...
      $("#browse-rows").innerHTML = entries.map((e) => `<tr data-name="${esc(e.name)}" data-dir="${e.dir ? 1 : 0}"><td class="ico">${e.dir ? "▸" : "·"}</td><td class="nm">${esc(e.name)}</td><td class="kd">${kind(e)}</td><td class="sz">${fmtSize(e.size)}</td><td class="mt">${fmtDate(e.mtime)}</td></tr>`).join("") || `<tr><td colspan="5" class="hint" style="padding:16px">Nothing here that can be indexed.</td></tr>`;
