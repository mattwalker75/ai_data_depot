...
function vectorSearch(qvec, bundleIds, k) {
  const db = open();
  const dbmod = require("./db");
  const inList = bundleIds.map(() => "?").join(",");
  if (dbmod.vecLoaded) {
    try {
      ...
      const keep = new Set(db.prepare(`SELECT id FROM chunks WHERE id IN (${ids.map(() => "?").join(",")}) AND bundle_id IN (${inList})`).all(...ids, ...bundleIds).map((r) => r.id));
  ...
  const rows = db.prepare(`SELECT id, embedding FROM chunks WHERE bundle_id IN (${inList}) AND embedding IS NOT NULL`).all(...bundleIds);
...
function keywordSearch(q, bundleIds, k) {
  const db = open();
  const match = ftsQuery(q);
  if (!match) return [];
  const inList = bundleIds.map(() => "?").join(",");
  try {
    return db.prepare(`SELECT c.id, bm25(chunks_fts) AS s FROM chunks_fts f JOIN chunks c ON c.id=f.rowid WHERE chunks_fts MATCH ? AND c.bundle_id IN (${inList}) ORDER BY s LIMIT ?`)
      .all(match, ...bundleIds, k).map((r, i) => ({ id: r.id, rank: i + 1, score: -r.s }));
  } catch { return []; }
}

/** Hybrid search -> [{chunk, document, bundle, score}] best first. */
async function search(query, { bundleIds, k = 12 } = {}) {
  if (!bundleIds || !bundleIds.length) return [];
  const [qvec] = await providers.embed([query]);
  const vec = vectorSearch(qvec, bundleIds, k * 2);
  const kw = keywordSearch(query, bundleIds, k * 2);
  const fused = new Map();
